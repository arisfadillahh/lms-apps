...
type Period = {
    ...
    total_amount: number;
    status: string;
    payment_plan_id?: string; // Need these to pre-fill
    pricing_id?: string;
};

type Props = {
    period: Period; // Extended period with IDs
    coderName: string;
    plans: PaymentPlan[];
    pricing: Pricing[];
};

export default function EditPaymentButton({ period, coderName, plans, pricing }: Props) {
    const router = useRouter();
    const [open, setOpen] = useState(false);
    const [isPending, startTransition] = useTransition();

    // In a real app, we might need these IDs passed in `period`. 
    // Assuming `period` object passed from parent HAS these fields from the join.
    // If tableData in page.tsx mapping omitted them, I need to update page.tsx first.
    // I will check page.tsx fetch: `.select('*, payment_plans(*), pricing(*)')`. 
    // So `period` passed to `CoderPaymentTable` should ideally have them.
    // I need to update `CoderPaymentTable` types and mapping.

    // Defaulting to empty or period's values
    const [planId, setPlanId] = useState(period.payment_plan_id || '');
    const [pricingId, setPricingId] = useState(period.pricing_id || '');
    const [startDate, setStartDate] = useState(period.start_date);
    const [error, setError] = useState<string | null>(null);

    // If props change, update state (sync)
    useEffect(() => {
        if (open) {
            setPlanId(period.payment_plan_id || '');
            setPricingId(period.pricing_id || '');
            ...
            {open && (
                <div style={backdropStyle} onClick={handleClose}>
                    <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
                        <h3 style={titleStyle}>Edit Periode Belajar</h3>
                        <p style={subtitleStyle}>Siswa: <strong>{coderName}</strong></p>
