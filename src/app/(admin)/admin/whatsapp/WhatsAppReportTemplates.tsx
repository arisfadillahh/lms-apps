...
const SAMPLE_DATA: Record<string, string> = {
    ...
    link_raport: 'https://lms.clevio.co/report/1a2b3c',
};
...
export default function WhatsAppReportTemplates({ settings, settingsMsg, savingSettings, onSettingChange, onSave }: WhatsAppTemplatesProps) {
    ...
    async function fetchTemplate() {
        try {
            ...
            const found = (data.templates || []).find((t: Template) => t.category === 'REPORT_SEND');
    ...
    async function handleSave() {
        ...
        try {
            const res = await fetch('/api/admin/whatsapp/templates', {
                method: 'POST', // API route handles upsert
            ...
            <div style={cardStyle}>
                ...
                <div style={variablesBox}>
                    ...
                    <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 8 }}>
                        * Variabel <code style={{ fontSize: 10 }}>{`{link_raport}`}</code> akan otomatis ditambahkan di akhir jika tidak disertakan dalam template.
                    </p>
                </div>
