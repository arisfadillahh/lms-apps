...
export default function EkskulRubricForm({ classId, semesterTag, coders, competencies, positiveCharacters }: EkskulRubricFormProps) {
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      const response = await fetch('/api/coach/rubrics/ekskul', {
        method: 'POST',
              ...
              {GRADES.map((grade) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '0.5rem',
                    border: '1px solid',
                    borderColor: grades[key] === grade ? '#2563eb' : '#cbd5f5',
                    background: grades[key] === grade ? '#2563eb' : '#fff',
                    color: grades[key] === grade ? '#fff' : '#2563eb',
                    fontWeight: 600,
                    cursor: 'pointer',
                  }}
                >
