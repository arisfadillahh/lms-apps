        ...
        <>
            <aside className="coder-sidebar w-72 flex-shrink-0 border-r-4 border-dashed border-pastel-pink/50 bg-white flex-col justify-between hidden md:flex sticky top-0 h-screen">
                <div className="p-8">
                    ...
                    {/* Navigation */}
                    <nav className="flex flex-col gap-3" aria-label="Navigasi Coder">
                        {NAV_LINKS.map((link) => {
            ...
            <nav className="coder-mobile-nav md:hidden" aria-label="Navigasi Coder mobile">
                {NAV_LINKS.map((link) => {
                    const isActive = pathname.startsWith(link.href);
                    const Icon = link.icon;
                    ...
                    return (
                        <Link
                            ...
                            data-coder-nav={link.href}
                            prefetch={true}
                            className={`coder-mobile-nav-item ${isActive ? 'is-active' : ''}`}
                        >
