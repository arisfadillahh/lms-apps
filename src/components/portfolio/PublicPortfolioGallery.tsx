...
export default function PublicPortfolioGallery({ projects }: { projects: PublicProject[] }) {
  ...
  useEffect(() => {
    ...
    return () => { document.body.classList.remove('overflow-hidden'); window.removeEventListener('keydown', onKeyDown); };
  }, [selected]);
  ...
  return <>
    <section id="projects" className="relative z-10 mx-auto max-w-6xl px-5 py-24 sm:px-8 sm:py-28">
      <div className="mb-10 grid gap-4 sm:grid-cols-[90px_1fr] sm:gap-5"><div className="pt-2 text-xs font-black tracking-[.18em] text-clevio-green">02</div><div><h2 className="m-0 text-[clamp(3rem,6.6vw,5.1rem)] font-black uppercase leading-[.9] tracking-[-.052em]">Project Universe</h2><p className="mt-5 max-w-2xl text-base font-semibold leading-relaxed text-white/65">Setiap project adalah cerita tentang ide, proses, tantangan, dan hal baru yang dipelajari.</p></div></div>
...
function ProjectCard({ project, index, onOpen }: { project: PublicProject; index: number; onOpen: (project: PublicProject) => void }) {
  ...
  return <button type="button" onClick={() => onOpen(project)} aria-label={`Buka cerita project ${project.snapshot.title}`} style={style} className={`group relative min-h-[430px] overflow-hidden rounded-[2rem] border border-white/10 bg-white/[.05] text-left shadow-2xl transition duration-300 hover:-translate-y-1 hover:border-[color:var(--portfolio-accent)] ${spanClass}`}>
    ...
    <div className="absolute inset-x-6 bottom-6 z-10"><span className="text-[10px] font-black uppercase tracking-[.15em]" style={{ color: accent }}>{project.snapshot.projectType}</span><h3 className="mt-2 text-[clamp(2rem,4vw,3rem)] font-black leading-[.92] tracking-[-.045em]">{project.snapshot.title}</h3><p className="mt-3 max-w-xl text-sm font-semibold leading-relaxed text-white/65">{project.snapshot.summary}</p><div className="mt-5 flex items-center justify-between gap-4 border-t border-white/15 pt-4 text-xs font-black text-white/75"><span>View Project Story</span><span className="grid size-11 place-items-center rounded-full bg-white text-[#0e1740] transition group-hover:rotate-12"><ArrowUpRight size={20} /></span></div></div>
  </button>;
}
...
function ProjectModal({ project, onClose }: { project: PublicProject; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[1200] grid place-items-center bg-[#050a22]/85 p-3 backdrop-blur-xl sm:p-6" role="dialog" aria-modal="true" aria-labelledby="public-project-title" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <article className="max-h-[calc(100dvh-1.5rem)] w-full max-w-5xl overflow-y-auto rounded-[2rem] border border-white/15 bg-gradient-to-br from-[#172862] to-[#0e1740] text-white shadow-[0_40px_110px_rgba(0,0,0,.45)]">
