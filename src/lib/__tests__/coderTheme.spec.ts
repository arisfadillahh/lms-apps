...
describe('coder theme preference', () => {
  ...
  it('keeps Learning Journey readable with continuous routes and a phone-first vertical timeline', () => {
    ...
    expect(map).toContain('journey-mobile flex min-h-0 flex-col px-4 pb-6 pt-1 md:hidden');
    ...
    expect(map).toContain('const mobileBlocks = blocks;');
    expect(map).toContain('journey-mobile-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain');
    expect(map).toContain('journey-mobile-card-compact');
    expect(map).toContain('Geser ke atas / bawah');
